"""Health endpoint — ping THẬT cả 2 dịch vụ managed (Postgres · Qdrant).
...
async def liveness() -> dict:
    """Chỉ trả lời "tiến trình còn sống" — KHÔNG chạm Postgres/Qdrant. Đây là path để trỏ
    Health Check của Render (slice 13).

    VÌ SAO tách khỏi `/health`: Render gửi health check VÀI GIÂY MỘT LẦN, liên tục suốt thời gian
    service chạy. Nếu trỏ vào `/health` (ping cả 2 dịch vụ) thì riêng health check đã ~17k lượt/ngày,
    giữ Neon luôn thức và đốt sạch compute-hours của gói free — hệ thống tự làm hỏng mình khi KHÔNG
    có ai dùng. Ngoài ra health check phải hỏi "tiến trình còn sống không"; một dịch vụ phụ chập chờn
    KHÔNG phải lý do để Render restart cả backend.
    """
    return {"status": "ok"}


@router.get("/health", summary="Kiểm SÂU — ping 2 dịch vụ (dành cho người/chẩn đoán)")
